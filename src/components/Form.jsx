...
const Form = () => {
  // Cria os estados para controlar os dados inseridos nos campos da lista (incluindo a descrição opcional)
  const [task, setTask] = useState('');
  const [date, setDate] = useState('');
  const [priority, setPriority] = useState('');
  const [description, setDescription] = useState('');

  // Define a função que processa o envio dos dados da lista
  const handleSubmit = (e) => {
    // Interrompe o comportamento padrão para evitar que a página dê reload
    e.preventDefault();
    console.log({ task, date, priority, description });
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16 items-start">
          ...
          <div className="w-full flex flex-col gap-4">
            <div className="border-2 border-dashed border-slate-400 p-8 text-center text-slate-500 font-sans text-sm">
              Sua lista de tarefas aparecerá aqui...
            </div>
          </div>
